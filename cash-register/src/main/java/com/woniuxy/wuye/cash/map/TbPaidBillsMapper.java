...
public interface TbPaidBillsMapper {
    //增
    @Insert("insert into tb_paid_bills(num,house_name,house_owner,should_fees,relief_fees,not_fees,late_fees,relief_late_fees,offset_fees,advice_sum_fees,is_offset,is_wipe_zero,wipe_zero_fees,real_sum_fees,get_fees_style,fees_account,pay_user,accept_user,accept_time,accept_num,invoice_number,note,fees_items,pay_status) " +
            "values(#{num},#{houseName},#{houseOwner.clientId},#{shouldFees},#{reliefFees},#{notFees},#{lateFees},#{reliefLateFees},#{offsetFees},#{adviceSumFees},#{isOffset},#{isWipeZero},#{wipeZeroFees},#{realSumFees},#{getFeesStyle},#{feesAccount},#{payUser},#{acceptUser},#{acceptTime},#{acceptNum},#{invoiceNumber},#{note},#{feesItems},#{payStatus})")
    ...
    @Results(id = "TbPaidBillsMap",value = {
            @Result(property = "houseOwner.clientId",column = "house_owner")
    })
    ...
    @ResultMap("TbPaidBillsMap")
    List<TbPaidBills> getByCondition(ConditionVo conditionVo);
